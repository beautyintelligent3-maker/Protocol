...
def migrate():
    ...
    with engine.begin() as conn:
        ...
        conn.execute(
            text("ALTER TABLE employees ADD COLUMN IF NOT EXISTS staff_id VARCHAR UNIQUE;")
        )
    ...
    with SessionLocal() as db:
        ...
        employees = db.execute(
            text("SELECT id FROM employees WHERE staff_id IS NULL ORDER BY created_at ASC")
        ).fetchall()
